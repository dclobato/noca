...
| `NOCA_JUDGE_PROFILING_MAX_CPU_TIME_SEC` | `10` | Hard CPU-time ceiling applied to **each** profiling repetition before Auto-Limit metrics are collected, and enforced as such: no single execution may exceed it. A test case of *N* repetitions therefore has up to *N* times this in aggregate, and no one run can consume more than its share. Read by the judge, which enforces it, **and** by Web, which states both numbers on the Limits tab so an author knows what will fail a profiling run before spending one; it therefore lives in the shared `profiling` env layer rather than in either module's own. |
...
| `NOCA_JUDGE_PROFILING_REAPER_STALE_THRESHOLD_MINUTES` | `10` | Staleness threshold for **profiling** jobs only. Profiling needs its own because its runtime has a shape no submission has: it runs the reference implementation over every test case *repetitions* times, so where a submission is bounded by `n_test_cases × time_limit`, a profiling run is bounded by `n_test_cases × repetitions × runtime` — a factor of 10 for the languages whose `profiling_repetitions_default` is 10. Sharing the submission threshold meant a legitimately slow reference implementation on a many-case problem could be reaped mid-run and retried until it hit `NOCA_JUDGE_REAPER_MAX_REQUEUE_COUNT`. Must stay below `NOCA_JUDGE_LOCK_TTL_SECONDS`, which is validated at startup for both thresholds. |
