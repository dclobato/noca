...
| `NOCA_WEB_PUBLIC_PROBLEM_PACK_PATH` | *(empty)* | Optional cache directory for the public post-contest problem-set archives served by `GET /problem-set/{slug}.zip` (an unauthenticated endpoint). When set, it must be an absolute path; the directory is created at startup if missing, and must be readable and writable by the web process when it already exists. Each released contest's ZIP is built once into a temporary sibling file, published atomically by rename, and reused while its `<name>.sha256` sidecar digest matches the file on disk; concurrent first-hit requests are serialized by a per-slug lock, so one contest costs at most one build at a time per process. When unset, every download rebuilds the archive from scratch — acceptable in development, but a public deployment should set this so anonymous bursts cannot exhaust the database pool. There is no automatic invalidation: delete a contest's cached file to force a rebuild (e.g. after editing its problems post-release). |
