...
`NOCA_JUDGE_PRE_WARM_CONTAINERS=true` warms each language pool lazily when the
first submission for that language appears.
...
| `NOCA_JUDGE_WORKER_CONCURRENCY` | Caps worker slots and lazy warm creation concurrency through the pool manager's shared warm-up semaphore. |
...
| `NOCA_JUDGE_IMAGE_NAMING` | Selects canonical registry-ref construction when `NOCA_JUDGE_IMAGE_REGISTRY` is set: `path` uses `<registry>/judge-<language>:<slot>`, while `flat` uses `<registry>-judge-<language>:<slot>`. |
| `NOCA_JUDGE_IMAGE_TAG` | Adds an optional suffix to canonical compile and run tags before pull, persistence, and preflight. |
...
| `NOCA_STARTUP_TIMEOUT_SECONDS` | Bounds the initial waits for PostgreSQL and Valkey before Docker image synchronization and preflight begin. `0` skips those waits. |

## Key outcome

Startup never creates run containers solely because
`NOCA_JUDGE_PRE_WARM_CONTAINERS=true`. Startup still validates required images
for all active languages before accepting work.

## Test coverage

The startup matrix is covered by
`tests/autojudge/test_container_startup_options.py`. Those tests exercise the
image-sync and required-image preflight helpers with a fake Docker image store,
then verify that `PoolManager` does not warm pools at construction time and only
triggers lazy warming on first acquire when
...
`real_docker` and are opt-in because they build and remove a local scratch
image:
