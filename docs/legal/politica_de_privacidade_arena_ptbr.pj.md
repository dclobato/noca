...
O responsável legal pode **revogar esse consentimento a qualquer momento**, conforme o art. 8º, §5º, da LGPD, sem necessidade de justificativa. Para isso, basta utilizar o link enviado no e-mail de confirmação do consentimento; a revogação também pode ser solicitada ao DPO. Cada link é válido para uma única revogação e deixa de funcionar sempre que o consentimento é novamente concedido, quando o e-mail do responsável é alterado ou quando o Usuário completa 18 anos — nesses casos, um novo link é enviado ou a revogação deixa de ser aplicável.

**A revogação suspende a conta, mas não a exclui.** Após a revogação, o Usuário deixa imediatamente de conseguir acessar a Plataforma, as sessões abertas são encerradas e o nome do Usuário deixa de aparecer em qualquer área pública. As submissões, os resultados, as conquistas e o histórico são preservados, e a conta volta a funcionar caso o consentimento seja concedido novamente. A **exclusão** definitiva dos dados é um direito distinto e deve ser solicitada ao DPO, conforme a Cláusula 10.

Todo Usuário recebe, no cadastro, um **nome de usuário pseudônimo** atribuído automaticamente pela Plataforma. Nas áreas públicas — o painel inicial, as páginas de ranking, o perfil público e as estatísticas de problemas — a identificação do Usuário depende da sua idade:

- **Usuários com 18 anos ou mais** são identificados, por padrão, pelo **nome civil**, e podem optar a qualquer momento por exibir o nome de usuário pseudônimo em seu lugar, nas configurações do perfil.
- **Usuários com idade entre 13 e 18 anos** são identificados **exclusivamente** pelo nome de usuário pseudônimo. O nome civil de um adolescente não é publicado nessas áreas, e essa proteção não é uma preferência: não pode ser desativada pelo próprio Usuário nem por um administrador. Esses Usuários também não dispõem de página de perfil público. Ao completar 18 anos, a escolha passa a ser do Usuário, e nada é publicado automaticamente até que ele opte por isso.
- Quando a **data de nascimento não estiver registrada**, a Plataforma adota a proteção mais restritiva, tratando a conta como a de um adolescente.

A busca pública de usuários acompanha essa regra: um perfil protegido não pode ser localizado pelo nome civil, apenas pelo nome de usuário. O nome de usuário pode ser alterado pelo próprio Usuário, respeitado um intervalo mínimo entre alterações, para que a substituição de um pseudônimo não permita associá-lo ao anterior.

Essa proteção é avaliada a cada acesso e deixa de se aplicar automaticamente quando o Usuário completa 18 anos, sem que isso torne público qualquer dado por conta própria: a exibição do nome civil continua dependendo de uma escolha expressa do Usuário adulto.
