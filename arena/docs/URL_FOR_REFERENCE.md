...
| `POST /admin/problems/{problem_id}/edit` | `arena_admin_problem_update` | `problem_id=`; form `active_tab`, required `save_action` (`keep_editing`/`enable`/`disable`). Saves the definition and targets the publication state atomically; HTML 422 opens the first invalid field or reports a failed enablement gate | `admin_problem_save.py` |
