...
| `GET` | `/problems/{arena_number}` | Authenticated problem detail page. Shows statement (Markdown + Mermaid + KaTeX), sample test cases, resource limits, language selector with submit form, solve/attempt status, and a toggleable favorite heart. Optional query params carry list state back for the "Back to list" button: `back_page`, `back_search`, `back_sort_by`, `back_category_slugs`. When the problem belongs to an accepting problem set from a class the user is registered to, shows a banner and a "submit for the problem set" checkbox bound to the most urgent such set. For an `ARENA_JUDGE` who owns an ongoing class, also shows a collapsed-by-default accordion with existing problem-set assignments and eligible teacher-owned targets. Existing assignment names link to their problem-set management pages. Classes that have ended are excluded; a future class or set start date does not exclude a target, while a past deadline does. |
