...
| `POST` | `/auth/signup` | Validates signup input, applies auth throttling keyed by ASGI client IP and hashed email, creates an Arena user account when the email is new, records Terms of Service and Privacy Policy acceptance timestamp, sends activation and parental-consent emails when applicable, and redirects to login with the same success message for new and already registered emails. For an already-registered email it records a `signup_existing_account` security event and sends an out-of-band "you already have an account — log in or reset your password" email to the address owner instead of another activation email, keeping the response indistinguishable from a fresh signup. Lockouts return `429` with `Retry-After`. |
