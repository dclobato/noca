...
| `POST` | `/auth/resend-activation` | Re-sends the email confirmation link for the account whose ID is stored in the Starlette session by the login handler. Consumes the session key on first use. IP-scoped throttling caps repeated resends; over-limit requests flash a retry message. |
...
| `POST` | `/auth/2fa` | Process the 2FA verification form with auth throttling keyed by ASGI client IP and hashed account email. Validates the PENDING_2FA session token, verifies the supplied TOTP or backup code, records completed login history as `2fa` or `backup_code`, and on success issues a LOGIN JWT cookie before redirecting to the pending flow's safe `next` URL or dashboard. The issued LOGIN token preserves the original `session_started_at` and `remember_me` metadata, so rotation and the absolute cap continue to apply to the same session. Lockouts return `429` with `Retry-After`. |
...
| `POST` | `/auth/password-reset` | Sends a password-reset email or resets the password for a valid reset token. The request and token-completion paths are auth-throttled and lockouts return `429` with `Retry-After`. The request path keeps the same response copy for existing and unknown accounts. |
...
| `GET` | `/dashboard` | Public Arena dashboard. Displays Latest Problems (10 most recently created or edited) and the rating-backed Leaderboard (top 10 users), followed by the supported languages. The top three ranks use the shared gold, silver, and bronze medal artwork. No authentication required. |
...
| `POST` | `/user/profile/photo` | Update the current Arena user's profile photo. Accepts a pre-cropped multipart upload (`foto_cropada`, 2:3 portrait). Streaming stops at `NOCA_IMAGE_MAX_FILE_SIZE`; browser failures redirect back with a warning. Redirects to profile on success. |
| `POST` | `/user/profile/personal-data` | Authenticated JSON endpoint that atomically updates name, date of birth, country/subdivision, affiliation, preferred programming language, `prefered_language` locale (`en-US` or `pt-BR`), and optional `ranking_visible` / `public_profile` (bool; omit or send `null` to preserve the current value). The `public_profile` opt-in is silently coerced to False whenever the resulting `ranking_visible` is False. A changed date applies the age policy: users under 13 are deactivated, users aged 13–17 lose parental consent, and both minor outcomes invalidate all sessions and return a dashboard redirect. Adult changes preserve the existing parental-consent fields. Response includes `ranking_visible` and `public_profile` reflecting the persisted values. |
...
No login required. The user must be active and email-confirmed, with both
`ranking_visible=True` and `public_profile=True`; otherwise the route returns
404 without revealing the account's existence. `ArenaRole.ARENA_ADMIN` viewers
bypass these checks.

| Method | URL | Description |
|--------|-----|-------------|
| `GET` | `/profile/{user_id}` | Public Arena user profile. Single-page (no tabs) with three sections: (1) name + affiliation + location; (2) badges grid (at most 5 per row); (3) submission heatmap, inline rating-history chart with current/historical rating, and verdict + language distribution doughnut charts driven by `arena_user_statistics`. |
| `GET` | `/profile/{user_id}/rating-history.json` | JSON endpoint returning the user's rating history for the last 24 months as `{"history": [{"ts": ISO8601, "ts_display": "YYYY-MM-DD", "rating": int}]}`, ordered chronologically. Used by the inline rating chart. |
| `GET` | `/profile/{user_id}/submission-heatmap.json` | JSON endpoint returning the user's precomputed submission heatmap as `{"heatmap": [["YYYY-MM-DD", count], ...], "range_start": "YYYY-MM-DD", "range_end": "YYYY-MM-DD", "computed_at": ISO8601 or null}`. Covers the last 364 days. Returns an empty heatmap until the first worker cycle writes a snapshot. |
| `GET` | `/profile/{user_id}/statistics.json` | JSON endpoint returning the user's precomputed verdict + language distributions as `{"total_submissions": int, "verdicts": [{"verdict", "count"}], "languages": [{"language_id", "name", "count"}], "computed_at": ISO8601 or null}`. Returns an empty payload when no snapshot exists yet. |
...
| `POST` | `/admin/users/{user_id}/toggle-public-profile` | `admin_users_actions.py` | Toggle the `public_profile` opt-in flag. Enabling is blocked (with a flash error) when `ranking_visible` is False. Admin-only; password confirmation required. |
...
| `GET` | `/ranking/users` | Paginated user ranking (50/page). Query params: `search`, `page`. `search` runs the indexed candidate query in `identity_search_service`: PostgreSQL full-text plus substring and 3+-character fuzzy matching on the name, and substring-only matching on the email (an address is an exact identifier). Websearch operators apply — `"quoted phrase"`, `a OR b`, and `-term` keep only the full-text branch — while typed `%` and `_` are matched literally. Search only filters: global rank is computed via SQL `RANK()` over all eligible users before it is applied, so rank positions stay globally consistent and never renumber. |
