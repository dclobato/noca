...
"""Container healthcheck entrypoint for the AI Assistant worker."""
...
def heartbeat_is_healthy(*, now: float | None = None) -> bool:
    """Return whether the AI Assistant worker heartbeat file is present and fresh.
    ...
    try:
        ...
        return heartbeat_is_fresh(
            Path(settings.AI_HEARTBEAT_FILE),
            max_age_seconds=settings.AI_HEARTBEAT_STALE_SECONDS,
            now=now,
        )
    except Exception:
        return False
