...
"""``NOCA_WEB_SSE_*`` settings: defaults, environment overrides, and validation."""
...
def test_sse_env_names_resolve(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    settings = _make_settings(
        monkeypatch,
        tmp_path,
        NOCA_WEB_SSE_LIMIT_ENABLED="false",
        NOCA_WEB_SSE_MAX_PER_IP="3",
        NOCA_WEB_SSE_MAX_PER_USER="2",
        NOCA_WEB_SSE_CONNECTION_TTL_SECONDS="120",
        NOCA_WEB_SSE_TRUSTED_CIDRS=" 10.0.0.0/8 , ::1/128 ",
    )
...
@pytest.mark.parametrize(
    "name", ["NOCA_WEB_SSE_MAX_PER_IP", "NOCA_WEB_SSE_CONNECTION_TTL_SECONDS", "NOCA_WEB_SSE_MAX_PER_USER"]
)
...
def test_sse_trusted_cidrs_are_validated(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, value: str) -> None:
    with pytest.raises(ValidationError, match="NOCA_WEB_SSE_TRUSTED_CIDRS"):
        _make_settings(monkeypatch, tmp_path, NOCA_WEB_SSE_TRUSTED_CIDRS=value)
