...
from rating import healthcheck as healthcheck_module
...
def test_heartbeat_is_healthy_when_file_exists_and_is_recent(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_FILE", str(heartbeat))
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_STALE_SECONDS", 10.0)
...
def test_heartbeat_is_healthy_returns_false_for_missing_file(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_FILE", str(tmp_path / "missing"))
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_STALE_SECONDS", 10.0)
...
def test_heartbeat_is_healthy_returns_false_for_stale_file(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_FILE", str(heartbeat))
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_STALE_SECONDS", 30.0)
...
def test_main_maps_health_to_process_exit_code(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_FILE", str(heartbeat))
    monkeypatch.setattr(settings, "RATING_HEARTBEAT_STALE_SECONDS", 3600.0)
