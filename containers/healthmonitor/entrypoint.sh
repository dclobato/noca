...
# containers/healthmonitor/entrypoint.sh
#
# Wait for Valkey to be ready, then start the public health monitor server.
...
setup_runtime_user() {
    ...
    [[ "$target_uid" =~ ^[0-9]+$ ]] || {
        echo "[healthmonitor-entrypoint] PUID must be numeric: ${target_uid}" >&2
        exit 1
    }
    [[ "$target_gid" =~ ^[0-9]+$ ]] || {
        echo "[healthmonitor-entrypoint] PGID must be numeric: ${target_gid}" >&2
        exit 1
    }
    ...
    if [[ "$current_uid" != "0" ]]; then
        echo "[healthmonitor-entrypoint] startup requires root for ${target_uid}:${target_gid}" >&2
        exit 1
    fi
...
echo "[healthmonitor-entrypoint] waiting for Valkey..."
...
async def wait_for_valkey() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            await client.ping()
            await client.aclose()
            print(f"[healthmonitor-entrypoint] Valkey is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(f"[healthmonitor-entrypoint] Valkey not ready (attempt {attempt}/{MAX_ATTEMPTS}): {exc}")
...
if [[ "$#" -eq 0 ]]; then
    set -- noca-healthmonitor
fi

echo "[healthmonitor-entrypoint] starting: $*"
exec "$@"
