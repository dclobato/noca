...
setup_runtime_user() {
    ...
    local statement_dir="${NOCA_WEB_PROBLEM_STATEMENT_DIR:?NOCA_WEB_PROBLEM_STATEMENT_DIR must be set}"
    ...
    [[ "$target_uid" =~ ^[0-9]+$ ]] || {
        echo "[webapp-entrypoint] PUID must be numeric: ${target_uid}" >&2
        exit 1
    }
    [[ "$target_gid" =~ ^[0-9]+$ ]] || {
        echo "[webapp-entrypoint] PGID must be numeric: ${target_gid}" >&2
        exit 1
    }
    ...
    if [[ "$current_uid" != "0" ]]; then
        echo "[webapp-entrypoint] startup requires root to switch to ${target_uid}:${target_gid}" >&2
        exit 1
    fi
...
echo "[webapp-entrypoint] waiting for PostgreSQL..."
...
async def wait_for_db() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            await conn.close()
            print(f"[webapp-entrypoint] PostgreSQL is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(f"[webapp-entrypoint] PostgreSQL not ready (attempt {attempt}/{MAX_ATTEMPTS}): {exc}")
...
echo "[webapp-entrypoint] waiting for Valkey..."
/app/.venv/bin/python - <<'PY'
import asyncio
import os
from urllib.parse import quote
...
async def wait_for_valkey() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            await client.ping()
            await client.aclose()
            print(f"[webapp-entrypoint] Valkey is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(f"[webapp-entrypoint] Valkey not ready (attempt {attempt}/{MAX_ATTEMPTS}): {exc}")
            await asyncio.sleep(SLEEP_SECONDS)
        finally:
...
if [[ -n "${NOCA_WEB_UBERADMIN_USERNAME:-}" ]]; then
    echo "Bootstrapping UberAdmin..."
    python scripts/web/create_uberadmin.py
else
    echo "Skipping UberAdmin seed (NOCA_WEB_UBERADMIN_USERNAME is not set)"
fi
