...
setup_runtime_user() {
    ...
    [[ "$target_uid" =~ ^[0-9]+$ ]] || {
        echo "[aiassistant-entrypoint] PUID must be numeric: ${target_uid}" >&2
        exit 1
    }
    [[ "$target_gid" =~ ^[0-9]+$ ]] || {
        echo "[aiassistant-entrypoint] PGID must be numeric: ${target_gid}" >&2
        exit 1
    }
    ...
    if [[ "$current_uid" != "0" ]]; then
        echo "[aiassistant-entrypoint] startup requires root for ${target_uid}:${target_gid}" >&2
        exit 1
    fi
...
echo "[aiassistant-entrypoint] waiting for PostgreSQL..."
...
async def wait_for_db() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            await conn.close()
            print(f"[aiassistant-entrypoint] PostgreSQL is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(
                "[aiassistant-entrypoint] PostgreSQL not ready "
...
echo "[aiassistant-entrypoint] waiting for Valkey..."
/app/.venv/bin/python - <<'PY'
import asyncio
import os
from urllib.parse import quote
...
async def wait_for_valkey() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            await client.ping()
            await client.aclose()
            print(f"[aiassistant-entrypoint] Valkey is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(f"[aiassistant-entrypoint] Valkey not ready (attempt {attempt}/{MAX_ATTEMPTS}): {exc}")
            await asyncio.sleep(SLEEP_SECONDS)
        finally:
