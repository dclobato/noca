...
# containers/animator/entrypoint.sh
#
# Wait for PostgreSQL and Valkey, wait for the schema to reach the head this
# image expects, then start the Animator presentation server.
#
# The animator is a read-only presentation consumer of the shared schema, never
# its steward: stewardship belongs to the independently-deployable HTTP front
# doors (web and arena), whose entrypoints run scripts/run_migrations.py. Like
# the autojudge, rating, and aiassistant workers, the animator instead blocks on
# scripts/wait_for_migrations.py so a mismatched image can never drive the
...
setup_runtime_user() {
    ...
    [[ "$target_uid" =~ ^[0-9]+$ ]] || {
        echo "[animator-entrypoint] PUID must be numeric: ${target_uid}" >&2
        exit 1
    }
    [[ "$target_gid" =~ ^[0-9]+$ ]] || {
        echo "[animator-entrypoint] PGID must be numeric: ${target_gid}" >&2
        exit 1
    }
    ...
    if [[ "$current_uid" != "0" ]]; then
        echo "[animator-entrypoint] startup requires root for ${target_uid}:${target_gid}" >&2
        exit 1
    fi
...
echo "[animator-entrypoint] waiting for PostgreSQL..."
...
async def wait_for_db() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            await conn.close()
            print(f"[animator-entrypoint] PostgreSQL is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(
                "[animator-entrypoint] PostgreSQL not ready "
...
echo "[animator-entrypoint] waiting for Valkey..."
/app/.venv/bin/python - <<'PY'
import asyncio
import os
from urllib.parse import quote
...
async def wait_for_valkey() -> None:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            await client.ping()
            await client.aclose()
            print(f"[animator-entrypoint] Valkey is ready (attempt {attempt})")
            return
        except Exception as exc:  # noqa: BLE001
            last_error = exc
            print(
                "[animator-entrypoint] Valkey not ready "
                f"(attempt {attempt}/{MAX_ATTEMPTS}): {exc}"
            )
            await asyncio.sleep(SLEEP_SECONDS)
        finally:
...
echo "[animator-entrypoint] waiting for database schema to reach the latest migration..."
...
echo "[animator-entrypoint] starting: $*"
exec "$@"
