...
"""Container healthcheck entrypoint for the mailer worker."""
...
def heartbeat_is_healthy(*, now: float | None = None) -> bool:
    """Return whether the mailer worker heartbeat file is present and fresh.
    ...
    try:
        ...
        return heartbeat_is_fresh(
            Path(settings.MAILER_HEARTBEAT_FILE),
            max_age_seconds=settings.MAILER_HEARTBEAT_STALE_SECONDS,
            now=now,
        )
    except Exception:
        return False
