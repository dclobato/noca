...
This endpoint is that missing request. It is deliberately inert: no database
session, no Valkey, no request body. The rotation is a side effect of the request
being authenticated at all, since the global ``enforce_web_default_auth``
dependency calls ``mark_auth_refresh_eligible`` for every authenticated
non-public path. Keeping the path off the public allowlist is therefore the whole
