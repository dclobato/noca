...
from web.dependencies import UserMediaContext, get_user_media_context
...
async def user_avatar_by_id(request: Request, media_ctx: UserMediaContextDep) -> Response:
    """Serve a stored avatar or deterministic fallback."""
    user = media_ctx.target_user
    media = await get_user_media(media_ctx.session, user.id)
    ...
    return cast(
        Response,
        request.app.state.image_service.build_image_response(data, mime, cache_directive=directive, request=request),
    )


@router.get("/user/{user_id}/photo")
async def user_photo_by_id(request: Request, media_ctx: UserMediaContextDep) -> Response:
...
async def user_audio_by_id(request: Request, media_ctx: UserMediaContextDep) -> Response:
    """Serve a user's stored audio clip to an authorized viewer."""
    media = await get_user_media(media_ctx.session, media_ctx.target_user.id)
