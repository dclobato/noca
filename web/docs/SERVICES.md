...
- per-language and simple problem limits, and profiling-run orchestration
...
- `problem_fallback_limits(problem) -> EffectiveProblemLimits` — normalized snapshot of the problem's own limits (the UI calls these the *simple limits*) with `repetitions=1`
- `submitted_language_limits(languages, submitted_form, existing_limits) -> dict[str, LanguageLimitInput]` — extracts posted per-language limits and preserves repetitions for unchanged rows; a language with no existing row is created with the registry's `profiling_repetitions_default` (3 or 10), **not** with 1. That count decides how many times a case runs, not what the submitted time limit means
- `changed_effective_limits(problem, languages, *, before_overrides, after_overrides, before_fallback=None, after_fallback=None) -> dict[str, tuple[str, EffectiveProblemLimits, EffectiveProblemLimits]]` — computes which languages had an effective-limit change and whether it was explicit or driven by a change to the problem's own limits
- `upsert_language_limits(session, problem, limits) -> None` — re-writes per-language limit rows, preserving stored repetitions when editing an existing row and defaulting new rows from the language registry. Rows are rewritten wholesale: a language left blank keeps no row and judges from the problem's own simple limits at 1 repetition
- `apply_fallback_limits(session, problem) -> bool` — copies separate `MAX()` values from `problem_language_limits` into `problems`; judging from the problem's own limits still uses exactly 1 repetition, and since every `time_limit_ms` is a per-run value the copied maximum is directly comparable
...
- `compute_profiling_limits_map(profiling_runs) -> dict[str, dict[str, int | None]]` — recomputes the Auto-Limit suggestion for the Limits tab from stored case results, through the shared `profiled_time_limit_ms()` so it cannot drift from what the worker persisted; divides by the run's own frozen `repetitions`
- `enqueue_profiling_job(valkey_runtime, profiling_run) -> None`

> **`time_limit_ms` is always the time for one run of a test case.** That holds for a per-language row and for the problem's own columns alike — the UI calls the latter the *simple limits*, and they are the one-repetition case of the same rule. The judge multiplies by the row's `repetitions` (`autojudge.types.case_budget_ms`) to get the budget for the whole case, then spends that one budget across the runs rather than re-imposing the limit on each: a slow run borrows from a fast one, so TLE means the **average** run went over, not that a single unlucky one did. That is exactly what makes repetitions smooth measurement noise instead of giving a submission N chances to fail. Memory, PID and output limits are peaks rather than sums and are unaffected by the count.

> **Limit-change batch snapshots predating the per-run change are historical.** `EffectiveProblemLimits.as_dict()` snapshots stored in `problem_limit_change_batch_languages` before the `202609070001` migration recorded `time_limit_ms` under the old meaning — a budget shared across repetitions. They are an audit record of what was decided at the time and are not rewritten, so a very old batch review page shows numbers on the old scale.
