...
- `submitted_language_limits(languages, submitted_form, existing_limits) -> dict[str, LanguageLimitInput]` — extracts posted per-language limits and preserves repetitions for unchanged rows; a language with no existing row is created with the registry's `profiling_repetitions_default` (3 or 10), **not** with 1
...
> **Repetitions change what `time_limit_ms` means.** It is the budget shared by *all* repetitions of a test case (`autojudge/submission_job.py` spends one `remaining_budget_ms` across the loop). The fallback path is fixed at 1 repetition, so its value is also the per-run budget; a per-language row carries its own count, and a hand-created one inherits the language registry's profiling default. The same number therefore means 1000 ms in the failover field and 100 ms per run in a 10-repetition language row. Hand-entered per-language limits — the only kind a running contest allows — must be given as per-run time × repetitions. Auto-Limit rows are self-consistent, since they measure across their own count. Memory, PID and output limits are peaks rather than sums and are unaffected.
